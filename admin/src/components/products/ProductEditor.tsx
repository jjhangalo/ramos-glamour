        ...
        <div className="flex">
          {(["produto", "variantes"] as const).map((tab) => (
            ...
            >
              ...
              {activeTab === tab && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-brand-midnight" />}
            </button>
          ))}
        </div>
      </nav>
          ...
          <div className="rounded-xl bg-white p-6 shadow-sm space-y-6">
            ...
            {!product ? (
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="text-sm text-brand-midnight/40 text-slate-500">Guarda o produto primeiro para activar os uploads.</p>
              </div>
            ) : (
                ...
                {product.product_variants?.map((variant) => (
                  <div key={variant.id}>
                    ...
                    {isDesktop && expandedVariantId === variant.id && (
                      <div className="bg-brand-bg/10 px-6 py-8 border-t border-brand-midnight/5">
